...
namespace ocropus {
    ...
    // in stack 'matches'
    bool WhitespaceCover::WState::is_done(WhitespaceCover *env) {
    ...
    int WhitespaceCover::WState::max_centricity(WhitespaceCover *env) {
        int mi = -1;
        float mv = -1.0;
        for(int i=0;i<matches->length();i++) {
            int index = matches->at(i);
    ...
    void WhitespaceCover::compute(rectarray &whitespaces,
        ...
        for(int i=0; i<obstacles.length(); i++) {
            if(obstacles[i].area())
                add_rect(rectangle(obstacles[i]));
        }
        //snug_bounds();
        compute();
    ...
    bool WhitespaceCover::good_dimensions(CState &result){
    ...
    void WhitespaceCover::generate_child_states(CState &state, 
    ...
    void WhitespaceCover::compute() {
        ...
        for(int iter=0;;iter++) {
            ...
            if(state->is_done(this)) { 
                if(!good_dimensions(state))
                    continue;
            ...
            int index = state->max_centricity(this);
            rectangle &r = rects[index];
            generate_child_states(state, r);
        }
    }
