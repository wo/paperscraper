...
namespace imglib {
    ...
    template<class T> void stretch(
            floatarray &out /* should be set to the new size */,
            const narray<T> &in) {
        ASSERT(out.rank() == 1);
        ASSERT(in.rank() == 1);
        fill(out, 0);
    ...
    template<class T> void load_row(narray<T> &row, const narray<T> &a,
            int index0) {
        for (int i = 0; i < a.dim(1); i++)
            row(i) = a(index0, i);
    }

    template<class T> void store_row(narray<T> &a, const narray<T> &row,
            int index0) {
        for (int i = 0; i < a.dim(1); i++)
            a(index0, i) = row(i);
    }

    template<class T> void stretch_rows(floatarray &dest, const narray<T> &src) {
        floatarray row_dest;
        narray<T> row_src;
    ...
    template<class T> void trim_range(bytearray &a, narray<T> &c) {
        makelike(a, c);
