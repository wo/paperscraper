...
namespace ocropus {
...
#if 0
    /// A direct interface to Tesseract to bypass oversegmentation.
    struct ISimpleLineOCR : colib::IComponent {
        /// Recognize the text line given by the grayscale image.
        virtual void recognize(colib::nustring &result,
                               colib::floatarray &costs,
                               colib::narray<colib::rectangle> &bboxes,
                               colib::bytearray &image) = 0;
        /// Adapt on a single character.
        virtual void adapt(colib::bytearray &image, colib::nustring &text)
        // FIXME maybe add a "get character training images" method here
    };
    
    
    /// \brief Make a wrapper for Tesseract.
    /// \warning There's really only one Tesseract,
    /// so instances would train each other!
    ///     (but this is not a singleton so that autodel<>
    ///      can be used normally).
    ///
    /// The returned bounding boxes may be 1-2 pixel inaccurate
    /// due to Tesseract's polygonal approximaion.
    ///
    /// The same costs are returned for every character in a word,
    /// because the Tesseract returns a struct
    /// that contains only the whole word's rating.
    ///
    /// @param path_to_our_executable Path to the executable (just argv[0]).
    ///                               Used to find tessdata/ directory.
    ISimpleLineOCR *make_tesseract(const char *path_to_our_executable = "");
#endif
    ...
    public:
        ...
        colib::rectangle bbox(int index) {
            return line_bboxes[index];
        }
        /// Set the bounding box of a given line
        void setBbox(colib::rectangle bbox, int index) {
            line_bboxes[index] = bbox;
        }
    };
