...
namespace ocropus {
    ...
    void WhitespaceCuts::get_whitespace_cuts(rectarray &wcuts,
                                             CharStats &charstats){
        wcuts.clear();
        rectarray hspaces,columns,colcandidates;
        ...
        max_space = 30;
        width     = 1.5;
        find_gutters(colcandidates,whitespaceboxes,charstats);
        filter_overlaps(columns,colcandidates);
    ...
    void SegmentPageByWCUTS::segment(intarray &image,bytearray &in_not_inverted) {
        ...
        rectarray page_blocks;
        page_seg->set_greedy(true);
        page_seg->set_maxresults(100);
        page_seg->set_minweight(5000);
        page_seg->set_minwidth(1);
        page_seg->set_minheight(1);
        ...
        if(debug_tiseg_intermediate){
            ...
            write_png_rgb(stdio("whitespace-cuts.png","w"),recolored_image);
        }
    }
