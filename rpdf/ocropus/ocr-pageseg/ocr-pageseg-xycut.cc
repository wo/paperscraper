...
namespace ocropus {
    ...
    void compute_integral_image(bytearray& img, 
    ...
    void get_projection_profiles(intarray& proj_on_yaxis, 
                                 intarray& proj_on_xaxis,
                                 rectangle& r, 
                                 intarray& int_img_hor, 
                                 intarray& int_img_ver) {
        // Mao. Fig.3. 2.a)
        proj_on_yaxis.resize((int)r.y1 - (int)r.y0 + 1) ;
        fill(proj_on_yaxis,0);

        proj_on_xaxis.resize((int)r.x1 - (int)r.x0 + 1) ;
        fill(proj_on_xaxis,0);

        for (int y = (int)r.y0; y < (int)r.y1; y++)
            proj_on_yaxis(y - (int)r.y0) = int_img_hor((int)r.x1, y) -
                                        int_img_hor((int)r.x0, y) ;

        for (int x = (int)r.x0; x < (int)r.x1; x++)
            proj_on_xaxis(x - (int)r.x0) = int_img_ver(x, (int)r.y1) - 
                                        int_img_ver(x,(int)r.y0) ;
    }


    // shrink to the bounding boy of all the black pixels in the rectangle and
    // remove noise
    void shrink_and_clean(rectangle& r, 
        ...
        while (xbegin >= 0 && proj_on_xaxis(xbegin) <= 0)
            xbegin++ ;
        while (xend >= 0 && proj_on_xaxis(xend) <= 0)
            xend-- ;
        
        int ybegin = 0, yend = proj_on_yaxis.dim(0) - 1 ;
        while (ybegin >= 0 && proj_on_yaxis(ybegin) <= 0)
            ybegin++ ;
    ...
    // projection profiles
    void get_widest_gap(int& cut_pos_y, 
                        int& gap_y,
                        int& cut_pos_x, 
                        int& gap_x,  
                        intarray& proj_on_yaxis,
                        intarray& proj_on_xaxis) {
        int gap_hor = 0, gap_ver = 0, pos_hor = 0, pos_ver = 0 ;
        int begin = -1 ;
        int end = -1 ;
        // find gap in y-axis projection
        for (int i = 0; i < proj_on_yaxis.dim(0); i++) {
            if (i > 0 
                && i < proj_on_yaxis.dim(0) - 1 
                && proj_on_yaxis(i-1) <= 0 
                && proj_on_yaxis(i) > 0)
                end = i ;
            if (i > 0 && proj_on_yaxis(i-1) > 0 && proj_on_yaxis(i) <= 0)
                begin = i ;
        ...
        for (int i = 0; i < proj_on_xaxis.dim(0); i++) {
            if (i > 0 
                && i < proj_on_xaxis.dim(0) - 1 
                && proj_on_xaxis(i-1) <= 0 
                && proj_on_xaxis(i) > 0)
                end = i ;
            if (i > 0 && proj_on_xaxis(i-1) > 0 && proj_on_xaxis(i) <= 0)
                begin = i ;
            if (begin > 0 && end > 0 && end - begin > gap_hor) {
                gap_ver = end - begin ;
                pos_ver = (end + begin) / 2 ;
    ...
    void insertRectToTree(int index, 
                          rectarray& tree, 
                          rectangle rL, 
                          rectangle rR) {
        int left = index * 2 + 1 ;
    ...
    void xycut(rectarray& blocks, 
        ...
        rectangle page = rectangle(0, 0, int_img_ver.dim(0) - 1,
                        int_img_ver.dim(1) - 1) ;
...
//                 int dir = -1 ;
            ...
            // done (0 = hor, 1 = ver)
                ...
                // insert leave rect to result blocks
    ...
    void SegmentPageByXYCUTS::segment(intarray &image,bytearray &in) {
        // check if image is binarized, if not do it
        binarize(in) ;
        ...
        if(debug_xycut_intermediate){
            ...
            invertbg(image);
            write_png_rgb(stdio("xycuts-cuts.png","w"),image);
            write_png_rgb(stdio("xycuts-inthor.png","w"),int_img_hor);
            write_png_rgb(stdio("xycuts-intver.png","w"),int_img_ver);
        }
    }
