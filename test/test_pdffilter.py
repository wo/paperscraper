...
import pytest
import os.path
from spamfilter import pdffilter
import scraper
...
def test_gooddoc():
    ...
    assert pdffilter.evaluate(doc, debug=1) > 0.98
...
def test_gooddoc_badlink():
    ...
    doc.numpages = 22
    doc.meta_confidence = 92
    assert pdffilter.evaluate(doc, debug=1) < 0.6
...
def test_cv():
    ...
    doc.numpages = 22
    doc.meta_confidence = 92
    assert pdffilter.evaluate(doc, debug=1) < 0.4
