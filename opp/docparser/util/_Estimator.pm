...
sub test {
    ...
    # some of the remaining distance towards 1 (or 0). Afterwards, I
	...
	if ($score >= 0) {
            # if $pos is 0.5, the new value $pos' should be
            # $pos+($score-0.5) = $score. If $pos > 0.5, we need to
            # scale the increment ($score-0.5), so that its new value
            # stands to the remainder (1-$pos) the way ($score-0.5)
            # stands to (1-0.5). I.e. $incr/(1-$pos) =
            # ($score-0.5)/0.5; i.e. $incr = ($score-0.5) * 2 *
            # (1-$pos).
	    $pos += ($score-0.5) * 2 * (1-$pos);
